 ...
 * ```
 */
export function loadJsonModules(context) {
    const modules = {};
 ...
 * ```
 */
export function loadModulesWithTransform(context, transformName) {
    const modules = {};
    context.keys().forEach((key) => {
        const baseName = key
            .replace('./', '')
 ...
 * ```
 */
export function getModuleNames(context) {
    return context.keys().map((key) => key
        .replace('./', '')
        .replace(/\.(json|js|ts|tsx)$/, ''));
}
